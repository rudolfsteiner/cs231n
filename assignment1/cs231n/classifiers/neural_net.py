...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    log_probs = -np.log(probs[range(num_train), y]+stab_factor)
    data_loss = np.sum(log_probs)/num_train 
    ...
    dW2 = np.dot(hidden1.T, dscore)
    db2 = np.sum(dscore, axis = 0, keepdims = True)
    
    dhidden1 = np.dot(dscore, W2.T)
  ...
  def predict(self, X):
    ...
    scores = np.dot(hidden1, self.params["W2"]) + self.params["b2"]
